...
class LoggerCallback(TrainCallback):
    ...
    def on_train_epoch_begin(self, trainer: BaseTrainer, func, params: Params, *args, **kwargs):
        ...
        trainer.logger.info("{}/{}".format(params.eidx, params.epoch), prefix="train epoch")

    def on_train_epoch_end(self, trainer: BaseTrainer, func, params: Params, meter: Meter, *args, **kwargs):
        self.traintime.mark("epoch")
        self.epochtime.end()

        avg = self.traintime["use"]/(params.eidx/self.start)
        last = (params.epoch - params.eidx) * avg

        trainer.logger.info("@train={} @epoch={} @avg={} @last={}".format(
            format_second(self.traintime["use"]),
            format_second(self.epochtime["use"]),
            format_second(avg),
            format_second(last),
    ...
    def on_train_batch_end(self, trainer: BaseTrainer, func, params: Params, meter: Meter, *args, **kwargs):
        if meter is None:
            ...
            if self.avg:
                ...
                meter = self.meter
        trainer.logger.inline("{}/{}".format(params.idx, len(trainer.iter_train_dataloader())), meter, fix=1)
...
class AutoReport(TrainCallback):
    ...
    def on_test_end(self, trainer: BaseTrainer, func, params: Params, meter: Meter, *args, **kwargs):
        if isinstance(meter,Meter):
            for k, v in meter.numeral_items():
                trainer.reporter.add_scalar(v, params.eidx, "test:{}".format(k))
    ...
    def on_eval_end(self, trainer: BaseTrainer, func, params: Params, meter: Meter, *args, **kwargs):
        if isinstance(meter, Meter):
            for k,v in meter.numeral_items():
                trainer.reporter.add_scalar(v,params.eidx,"eval:{}".format(k))

    def on_train_epoch_end(self, trainer: BaseTrainer, func, params: Params, meter: AvgMeter, *args, **kwargs):
        if isinstance(meter, Meter):
            for k, v in meter.numeral_items():
                trainer.reporter.add_scalar(v, params.eidx, k)

    def report(self, trainer, params, e=None):
        from thexp.utils.markdown_writer import Markdown
        md = Markdown()
        with md.code() as code:  # type:Markdown
            code.add_line(str(params))

        if e is not None:
            with md.quote() as quote:
                quote.add_text("Early Stoped because of **{}**".format(e.__class__.__name__))

        for tag,f in [[os.path.splitext(k)[1],k] for k in trainer.experiment.exp_info["files"].keys()]:
            trainer.reporter.add_ftag(f,tag)

        fn = trainer.reporter.report((params.eidx-self.start) // 20, otherinfo=md)
        trainer.logger.info("Report saved in \n\t{}".format(fn))

    def on_first_exception(self, trainer: BaseTrainer, func, params: Params, e: BaseException, *args, **kwargs):
        if (params.eidx-self.start) <= 2:
            return
        self.report(trainer, params, e)

    def on_train_end(self, trainer: BaseTrainer, func, params: Params, meter: Meter, *args, **kwargs):
        self.report(trainer, params)
